...
class McpService {
    ...
    read_mcp_config() {
        ...
        if (!public_1.pub.file_exists(mcp_config_file)) {
            ...
            ee_core_adapter_1.logger.info(`MCP配置文件 ${mcp_config_file} 不存在，已创建默认配置文件`);
        }
        try {
            return public_1.pub.read_json(mcp_config_file);
        }
        catch (error) {
            ee_core_adapter_1.logger.error(`读取 MCP 配置文件 ${mcp_config_file} 时出错:`, error);
            return null;
        }
    }
    /**
    ...
    save_mcp_config(mcpServers) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error(`保存 MCP 配置文件 ${mcp_config_file} 时出错:`, error);
        }
    }
    /**
     * 获取常用的MCP服务器列表
    ...
    read_common_mcp_config() {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error(`读取常用 MCP 配置文件 ${common_mcp_config_file} 时出错:`, error);
            return null;
        }
    }
    /**
    ...
    save_common_mcp_config(config) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error(`保存常用 MCP 配置文件 ${common_mcp_config_file} 时出错:`, error);
        }
    }
    get_bin_path() {
    ...
    async download_file(url, saveFile) {
        ...
        try {
            ...
            if (contentLength && downloadBytes >= parseInt(contentLength) || response.status === 416) {
                ee_core_adapter_1.logger.info(`文件 ${saveFile} 已经下载完成，跳过下载`);
                return true;
            }
            // 检查响应状态码
            if (response.status !== 200 && response.status !== 206) {
                ee_core_adapter_1.logger.error(`下载文件失败，状态码: ${response.status}`);
                return false;
            }
    ...
    save_mcp_tools(name, tools) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error(`保存 MCP 工具文件 ${mcpToolsFile} 时出错:`, e);
        }
    }
    /**
     * 读取 MCP 工具列表
    ...
    read_mcp_tools(name) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error(`读取 MCP 工具文件 ${mcpToolsFile} 时出错:`, e);
            return [];
        }
    }
    /**
