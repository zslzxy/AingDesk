...
class OllamaService {
    ...
    async version() {
        try {
            try {
                ...
                ee_core_adapter_1.logger.error('Get ollama version error:', e);
            }
            // 尝试从命令行获取版本信息
            ...
            ee_core_adapter_1.logger.error(public_1.pub.lang('获取 ollama 版本时出错:'), error);
            return '';
        }
    }
    /**
    ...
    async is_running() {
        try {
            ...
            await ollama.ps();
            ee_core_adapter_1.logger.info('Ollama is running');
            return true;
        }
        catch (e) {
            ee_core_adapter_1.logger.warn('Ollama is not running');
            return false;
        }
    }
    /**
    ...
    async model_list() {
        try {
            ...
            ee_core_adapter_1.logger.error(public_1.pub.lang('获取 Ollama 模型列表时出错:'), error);
            return [];
        }
    }
    /**
    ...
    download_speed_monitoring(fullModel) {
        ...
        if (average10s < average / 3) {
            ee_core_adapter_1.logger.warn(public_1.pub.lang('检测到下载速度异常，正在尝试重新连接下载节点...'));
            if (fullModel === 'ollama') {
    ...
    async install_model(model, parameters) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error(public_1.pub.lang('安装模型时出错:'), error);
            return false;
        }
    }
    /**
    ...
    async remove_model(model, parameters) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error(public_1.pub.lang('删除模型时出错:'), error);
            return null;
        }
    }
    async ollama_download_end(downloadFile) {
        try {
            ...
            if (installed) {
                ee_core_adapter_1.logger.info(public_1.pub.lang('安装完成'));
            }
        }
        catch (error) {
            ee_core_adapter_1.logger.error(public_1.pub.lang('安装过程中出现错误:'), error);
        }
    }
    /**
     * 安装 Ollama
    ...
    async install_ollama() {
        ...
        try {
            ...
            const handleError = async (error) => {
                ...
                ee_core_adapter_1.logger.error(public_1.pub.lang('下载过程中出现错误:'), error);
                OllamaDownloadSpeed.status = -1;
            };
            ...
            if (error.code === 'ERR_BAD_REQUEST' || error.status == 416) {
                ...
                ee_core_adapter_1.logger.error(public_1.pub.lang('安装过程中出现未知错误:'), error);
                return false;
            }
        }
    }
    ...
    set_ollama_model_save_path(save_path) {
        try {
            if (public_1.pub.is_windows()) {
                ...
                ee_core_adapter_1.logger.info(public_1.pub.lang('Windows 环境变量设置成功。'));
                // 结束 Ollama 进程
                this.killOllamaProcess('cmd.exe');
                // 重新启动 Ollama
                (0, child_process_1.exec)('"ollama app"');
                ee_core_adapter_1.logger.info(public_1.pub.lang('Ollama 已重新启动。'));
            }
            else if (public_1.pub.is_linux()) {
                // 修改 Linux 环境变量
                ...
                process.env.OLLAMA_MODELS = save_path;
                ee_core_adapter_1.logger.info(public_1.pub.lang('Linux 环境变量设置成功。'));
                // 结束 Ollama 进程
                ...
                (0, child_process_1.execSync)(`echo "${configContent}" >> ${shellConfigFile}`);
                process.env.OLLAMA_MODELS = save_path;
                ee_core_adapter_1.logger.info(public_1.pub.lang('macOS 环境变量设置成功。'));
                // 结束 Ollama 进程
                ...
                ee_core_adapter_1.logger.info(public_1.pub.lang('不支持的操作系统'));
                return false;
            }
            return true;
        }
        catch (error) {
            ee_core_adapter_1.logger.error(public_1.pub.lang('设置 Ollama 模型存储目录或重启服务时出错:'), error);
            return false;
        }
    }
    /**
            ...
            const checkInstallStatus = () => {
                setTimeout(async () => {
                    ...
                    if (version.length > 0) {
                        ...
                        ee_core_adapter_1.logger.info('Ollama install successify');
                        public_1.pub.delete_file(downloadFile);
                        resolve(true);
                    }
                    else {
                        OllamaDownloadSpeed.status = -1;
                        ee_core_adapter_1.logger.error('Ollama install failed');
                ...
                if (public_1.pub.is_windows()) {
                    ...
                    (0, child_process_1.exec)('setx OLLAMA_HOST "127.0.0.1"', () => {
                        (0, child_process_1.exec)(`"${downloadFile}" /SILENT /NORESTART`, (error, stdout, stderr) => {
                            if (error) {
                                ee_core_adapter_1.logger.error(`ollama install error: ${error.message}`);
                                resolve(false);
                            }
                            if (stderr) {
                                ee_core_adapter_1.logger.error(`ollama install stderr: ${stderr}`);
                                resolve(false);
                            }
                            if (stdout) {
                                ee_core_adapter_1.logger.info(`ollama install stdout: ${stdout}`);
                            }
                            OllamaDownloadSpeed = {
                                total: 0,
                                completed: 0,
                    ...
                    try {
                        ...
                        ee_core_adapter_1.logger.error('Error during Ollama DMG installation:', error);
                        resolve(false);
                    }
                }
                else if (public_1.pub.is_linux()) {
...
`;
                        ...
                        ee_core_adapter_1.logger.error('Error during Ollama Linux installation:', error);
                        resolve(false);
                    }
                }
            }, 4000);
    ...
    killOllamaProcess(shell) {
        try {
            ...
            ee_core_adapter_1.logger.info(public_1.pub.lang('Ollama 进程已结束。'));
        }
        catch (killError) {
            ee_core_adapter_1.logger.warn(public_1.pub.lang('结束 Ollama 进程时可能未找到进程:'), killError.message);
        }
    }
