...
const ee_core_adapter_4 = require("../../server/adapters/ee-core-adapter");
const ee_core_adapter_5 = require("../../server/adapters/ee-core-adapter");
/**
 * Window
 * @class
 */
class WindowService {
    ...
    createWindow(args) {
        ...
        let contentUrl = '';
        if (type == 'html') {
            contentUrl = path_1.default.join('file://', (0, ee_core_adapter_2.getBaseDir)(), content);
        }
        else if (type == 'web') {
            contentUrl = content;
        }
            ...
            if ((0, ee_core_adapter_2.isProd)()) {
                const { mainServer } = (0, ee_core_adapter_3.getConfig)();
                if (mainServer && mainServer.protocol && (0, ee_core_adapter_4.isFileProtocol)(mainServer.protocol)) {
                    addr = mainServer.protocol + path_1.default.join((0, ee_core_adapter_2.getBaseDir)(), mainServer.indexPath);
                }
            }
            contentUrl = addr + content;
        }
        ee_core_adapter_5.logger.info('[createWindow] url: ', contentUrl);
        const opt = {
            title: windowTitle,
            x: 10,
            y: 10,
    ...
    getWCid(args) {
        ...
        let win;
        if (windowName == 'main') {
            win = (0, ee_core_adapter_1.getMainWindow)();
        }
        else {
    ...
    communicate(args) {
        ...
        if (receiver == 'main') {
            const win = (0, ee_core_adapter_1.getMainWindow)();
