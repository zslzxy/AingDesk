        ...
        for (const contextId of chatList) {
            ...
            if (chatConfigBody) {
                try {
                    ...
                    ee_core_adapter_1.logger.error(`Failed to parse chat config file: ${chatConfigFile}`, error);
                }
            }
        }
        // 按照创建时间倒序排序
    ...
    getShareChatInfo(shareId, contextId) {
        ...
        if (chatHistoryBody) {
            try {
                ...
                ee_core_adapter_1.logger.error(`Failed to parse chat history file: ${chatHistoryFile}`, error);
                return public_1.pub.return_error(public_1.pub.lang('获取失败'), null);
            }
        }
        return public_1.pub.return_error(public_1.pub.lang('获取失败'), null);
...
[${public_1.pub.lang('用户文档')} ${idx} end]`).join("\n");
            ...
            if (isOllama) {
                ...
                try {
                    ...
                    ee_core_adapter_1.logger.error(public_1.pub.lang('调用模型接口时出错:'), error);
            ...
            ee_core_adapter_1.logger.error('Error while chatting with Ollama:', error);
    ...
    getShareInfo(shareId) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error(`Failed to parse share config file: ${shareConfigPath}`, error);
            return null;
        }
    }
    // 生成唯一的分享 ID 前缀
    ...
    handleReceivedData(conn, data) {
        try {
            ...
            ee_core_adapter_1.logger.error('Error while handling received data:', error);
    ...
    connectToCloudServer(shareIdPrefix) {
        ...
        const socket = tls_1.default.connect(options, () => {
            ee_core_adapter_1.logger.info('Connected to cloud server');
            // 发送分享 ID 前缀
            this.sendToServer(socket, shareIdPrefix, 0);
        });
        socket.on('timeout', () => {
            ee_core_adapter_1.logger.error('Connection timed out');
            socket.destroy();
        });
        if (socket)
        ...
        // 监听连接关闭事件
        socket.on('end', () => {
            ee_core_adapter_1.logger.info('Disconnected from cloud server');
        });
        // 监听错误事件
        socket.on('error', (error) => {
            ee_core_adapter_1.logger.error('Socket error:', error);
        });
        // 监听关闭事件
        socket.on('close', (hadError) => {
            if (hadError) {
                ee_core_adapter_1.logger.error('Socket closed with error');
            }
            else {
                ee_core_adapter_1.logger.info('Socket closed normally');
            }
        });
        return socket;
    }
