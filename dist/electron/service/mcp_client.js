...
class MCPClient {
    ...
    static async readMCPConfigFile() {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error('Failed to read MCP config file:', error);
        }
        return null;
    }
    /**
    ...
    static parseMCPConfig(configContent) {
        try {
            ...
            ee_core_adapter_1.logger.error('Failed to parse MCP config:', error);
            return [];
        }
    }
    /**
    ...
    async createSSETransport(url) {
        try {
            ...
            ee_core_adapter_1.logger.error(`Failed to create SSE transport for URL ${url}:`, error);
            throw error;
        }
    }
    /**
    ...
    async connectToServer(serverConfigList) {
        for (let serverConfig of serverConfigList) {
            try {
                ...
                ee_core_adapter_1.logger.error(`Failed to connect to server ${serverConfig.name}:`, error);
    ...
    async getTools(serverConfig) {
        try {
            ...
            ee_core_adapter_1.logger.error(`Failed to get tools for server ${serverConfig.name}:`, error);
            throw error;
        }
        finally {
            this.cleanup();
    ...
    async getAllAvailableTools() {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error("Failed to get available tools:", error);
            throw error;
        }
    ...
    async executeToolCall(session, toolName, toolArgs) {
        try {
            ...
            ee_core_adapter_1.logger.error(`Failed to execute tool call for ${toolName}:`, error);
            throw error;
        }
    }
    /**
    ...
    async handleToolCalls(availableTools, messages, isRecursive = false) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error("Failed to call OpenAI API:", error);
            throw error;
        }
    }
    /**
    ...
    async processQuery(openai, supplierName, model, messages, callback, push) {
        ...
        try {
            ...
            callback(chunk);
            ee_core_adapter_1.logger.error("Failed to process query:", error);
            throw error;
        }
        finally {
            // 关闭所有连接
    ...
    async cleanup() {
        try {
            ...
            ee_core_adapter_1.logger.error("Failed to cleanup connections:", error);
            throw error;
        }
    }
    /**
