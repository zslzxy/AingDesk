...
const checkIsVisionModel = async (supplierName, model) => {
    ...
    try {
        ...
        if (!public_1.pub.file_exists(modelListFile)) {
            ee_core_adapter_1.logger.warn('模型列表文件不存在:', modelListFile);
            return false;
        }
        const modelList = public_1.pub.read_json(modelListFile);
        if (!Array.isArray(modelList)) {
            ee_core_adapter_1.logger.warn('模型列表格式不正确');
            return false;
        }
        ...
        ee_core_adapter_1.logger.error('检查模型视觉能力时出错:', error);
        return false;
    }
};
// 提取保存对话内容的函数
        ...
        const ResEvent = async (chunk) => {
            ...
            if (!exports.ContextStatusMap.get(uuid)) {
                try {
                    ...
                    ee_core_adapter_1.logger.error('Abort error:', error.message);
                }
