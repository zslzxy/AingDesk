...
class LanceDBManager {
    ...
    static async optimizeTable(tableName) {
        try {
            ...
            ee_core_adapter_1.logger.error('优化表失败', e);
    ...
    static endMetrics(metrics) {
        ...
        ee_core_adapter_1.logger.info(`[性能] ${metrics.operation}: ${metrics.duration.toFixed(2)}ms`);
    }
    ...
    static async createTable(tableName, supplierName, model, initialText) {
        ...
        try {
            ...
            try {
                ...
                ee_core_adapter_1.logger.error('创建docId索引失败', e);
            }
            // tokens索引
            try {
                ...
                ee_core_adapter_1.logger.error('创建doc索引失败', e);
            }
            // keywords索引
            try {
                ...
                ee_core_adapter_1.logger.error('创建keywords索引失败', e);
            }
    ...
    static async createDocFtsIndex(tableName) {
        try {
            ...
            ee_core_adapter_1.logger.error('创建FTS索引失败', e);
        }
    }
    /**
     * 添加索引
    ...
    static async addIndex(tableName, indexKeys) {
        ...
        try {
            ...
            for (const indexKey of indexKeys) {
                ...
                if (indexStats) {
                    ee_core_adapter_1.logger.error(`索引 "${indexName}" 已存在，跳过创建`);
                    continue;
                }
                let indexConfig = null;
                switch (indexKey.type) {
            ...
            ee_core_adapter_1.logger.error('添加索引失败', e);
            return false;
        }
        finally {
            await db.close();
    ...
    static async deleteIndex(tableName, indexKey) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error('删除索引失败', e);
            return false;
        }
        finally {
            await db.close();
    ...
    static async checkColumn(tableObj, recordInfo) {
        ...
        // 添加字段
        ee_core_adapter_1.logger.info('添加字段', newFields);
    ...
    static async updateRecord(tableName, record) {
        ...
        try {
            ...
            await tableObj.optimize();
            ee_core_adapter_1.logger.info(`成功更新文档到表 ${tableName}`);
            return true;
        }
        catch (error) {
            ee_core_adapter_1.logger.error(`更新文档失败: ${error.message}`, tableName, record);
            return false;
        }
        finally {
            await db.close();
    ...
    static async deleteRecord(tableName, where) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error(`删除文档失败: ${error.message}`);
            return false;
        }
        finally {
            await db.close();
    ...
    static async queryRecord(tableName, where, field = []) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error(`查询文档失败: ${error.message}`);
            return [];
        }
        finally {
            await db.close();
    ...
    static async getDocName(docIdList) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error(`获取文档名称失败: ${error.message}`);
        }
        finally {
            this.endMetrics(metrics);
        }
...
// 向量维度
LanceDBManager.DIMENSION = 1024;
// 是否启用性能监控
LanceDBManager.ENABLE_METRICS = false;
