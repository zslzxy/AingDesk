...
class RagTask {
    ...
    async parse() {
        ...
        for (let doc of notParseDocument) {
            try {
                ...
                ee_core_adapter_1.logger.error(public_1.pub.lang('[parseDocument]解析文档失败'), e);
                await vector_lancedb_1.LanceDBManager.updateRecord(this.docTable, { where: `doc_id='${doc.doc_id}'`, values: { is_parsed: -1 } });
            }
        }
    }
    ...
    async embed() {
        try {
            ...
            for (let doc of notEmbeddingDocument) {
                ...
                for (let checkInfo of chunkList) {
                    try {
                        ...
                        ee_core_adapter_1.logger.error(public_1.pub.lang('[addDocument]插入数据失败'), e);
                        await vector_lancedb_1.LanceDBManager.updateRecord(this.docTable, { where: `doc_id='${doc.doc_id}'`, values: { is_parsed: -1 } });
                    }
                }
                // 更新文档状态
            ...
            ee_core_adapter_1.logger.error(public_1.pub.lang('[embed]嵌入文档失败'), e);
            return e;
        }
    }
}
