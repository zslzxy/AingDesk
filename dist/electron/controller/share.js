...
class ShareController {
    ...
    readJsonFile(filePath) {
        try {
            ...
            ee_core_adapter_1.logger.error(`读取并解析 JSON 文件 ${filePath} 时出错:`, error);
            return null;
        }
    }
    /**
    ...
    saveJsonFile(filePath, data) {
        try {
            ...
            ee_core_adapter_1.logger.error(`保存 JSON 文件 ${filePath} 时出错:`, error);
        }
    }
    /**
     * 获取指定分享配置
    ...
    async remove_share(args) {
        ...
        if (public_1.pub.file_exists(sharePath)) {
            try {
                ...
                ee_core_adapter_1.logger.error(`删除分享 ${args.share_id} 时出错:`, error);
            }
        }
    ...
    async create_share(args) {
        ...
        try {
            ...
            ee_core_adapter_1.logger.error(`创建分享目录 ${sharePath} 时出错:`, error);
