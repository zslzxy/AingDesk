...
class IndexController {
    ...
    async get_languages() {
        try {
            ...
            // 记录错误日志
            ee_core_adapter_1.logger.error(public_1.pub.lang('获取语言列表时出错:'), error);
            // 返回错误响应
    ...
    async set_language(args) {
        try {
            ...
            // 记录日志
            ee_core_adapter_1.logger.info(public_1.pub.lang('设置语言为: {}', language));
            // 定义需要清除的缓存键数组
            ...
            // 记录错误日志
            ee_core_adapter_1.logger.error(public_1.pub.lang('设置语言时出错:'), error);
            // 返回错误响应
    ...
    async get_client_language() {
        try {
            ...
            // 记录错误日志
            ee_core_adapter_1.logger.error(public_1.pub.lang('获取客户端语言包时出错:'), error);
            // 返回错误响应
    ...
    async get_server_language() {
        try {
            ...
            // 记录错误日志
            ee_core_adapter_1.logger.error(public_1.pub.lang('获取服务端语言包时出错:'), error);
            // 返回错误响应
    ...
    async select_folder(args, event) {
        ...
        let result = await dialog.showOpenDialog({
            ...
            title: public_1.pub.lang('选择目录'),
            message: public_1.pub.lang('请选择一个目录')
        });
        // 如果有选择目录
        if (!result.canceled) {
            // 返回成功响应
        ...
        return public_1.pub.return_error(public_1.pub.lang('未选择目录'));
    }
    /**
     * 接收前端错误日志，并写入到日志文件
     * @param args
    ...
    async write_logs(args) {
        const { logs } = args;
        // 记录日志
        ee_core_adapter_1.logger.error(logs);
